...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await getSurveyForWorkspace({ workspaceId: session.workspace!.id, surveyId: id })
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await updateSurvey({ workspaceId: session.workspace!.id, surveyId: id, updates })
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await deleteSurvey({ workspaceId: session.workspace!.id, surveyId: id })
  if (result.error) return NextResponse.json({ error: result.error.message }, { status: 500 })
