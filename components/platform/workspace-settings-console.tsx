...
export function WorkspaceSettingsConsole({
  ...
  async function save() {
    ...
    const response = await fetch("/api/dashboard/settings", {
      ...
      body: JSON.stringify({ name, logoLabel, themeColor, supportEmail })
    })
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-600">Manage the workspace identity shown across the app shell.</p>
      </div>
      ...
      <section className="grid gap-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="space-y-5">
          <div>
            <label className="text-sm font-semibold text-slate-700" htmlFor="workspace-name">Workspace name</label>
            <input
              id="workspace-name"
              value={name}
