...
export type PlanBillingType = "free" | "paid" | "grant_only"
...
export function resolveAvailablePlanPrice(plan: ResolvableBillingPlan, interval: BillingInterval) {
  if (!plan.active || plan.status !== "active" || plan.billing_type !== "paid") {
...
export function getPlanCatalogDisposition(input: {
  ...
  if (input.billingType !== "paid") return "not_applicable"
